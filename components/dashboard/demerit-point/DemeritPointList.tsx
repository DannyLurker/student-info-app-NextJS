...
interface DemeritPointListProps {
  data: DemeritPointRecord[];
}
...
export const DemeritPointList = ({ data }: DemeritPointListProps) => {
  if (data.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-[300px] text-gray-500">
        <AlertTriangle className="w-12 h-12 mb-2 opacity-20" />
        <p>No demerit points recorded</p>
      </div>
    );
  }
    ...
    <div className="space-y-3 max-h-[500px] overflow-y-auto pr-2">
      {sortedData.map((item, index) => {
        ...
        return (
          ...
          >
            <div
              className={`p-2 rounded-full flex-shrink-0 ${CATEGORY_COLORS[item.category] || "bg-gray-100"}`}
            >
