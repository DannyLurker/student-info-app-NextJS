...
export default function ProblemPointManagement({ session }: ProblemPointManagementProps) {
    ...
    const handleDelete = async () => {
        ...
        try {
            await axios.delete("/api/problem-point", {
                params: { problemPointId: deleteItem.id, teacherId: session.user.id }
            });
    ...
    return (
        <div className="space-y-6 p-4">
            ...
            <div className="bg-white p-4 rounded-xl border shadow-sm">
                <h3 className="text-sm font-medium mb-3 text-gray-500 uppercase tracking-wider">Filter by Class</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    >
                        ...
                        <SelectContent>
                            {GRADES.map((g) => (
                                <SelectItem key={g} value={g}>{GRADE_DISPLAY_MAP[g]}</SelectItem>
                            ))}
                        </SelectContent>
                    </Select>
                    <Select
                    ...
                    >
                        ...
                        <SelectContent>
                            {MAJORS.map((m) => (
                                <SelectItem key={m} value={m}>{MAJOR_DISPLAY_MAP[m]}</SelectItem>
                            ))}
                        </SelectContent>
                    </Select>
                    <Select
                        value={filters.classNumber}
                        onValueChange={(val) => setFilters({ ...filters, classNumber: val })}
                    >
                        <SelectTrigger>
                        ...
                        <SelectContent>
                            {CLASSNUMBERS.map((c) => (
                                <SelectItem key={c} value={c}>{c === "none" ? "None" : c}</SelectItem>
                            ))}
                        </SelectContent>
                    </Select>
                </div>
                ...
                {!isClassSelected ? (
                    ...
                    <div className="space-y-4">
                        {data.map((item) => (
                            ...
                            >
                                {/* Icon */}
                                <div className={`p-2 rounded-full h-fit ${CATEGORY_COLORS[item.category] || "bg-gray-100"}`}>
            ...
            <AlertDialog open={!!deleteItem} onOpenChange={(open) => !open && setDeleteItem(null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            This action cannot be undone. This will permanently delete the problem point record for <span className="font-bold">{deleteItem?.student?.name}</span>.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
