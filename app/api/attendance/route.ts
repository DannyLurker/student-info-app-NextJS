...
export async function POST(req: Request) {
  try {
    ...
    const attendanceMap = new Map<string, { id: number; studentId: string; type: ValidAttendanceType; note: string; date: Date }>(
      existingAttendances.map((a: { id: number; studentId: string; type: any; note: string | null; date: Date }) => [a.studentId, { id: a.id, studentId: a.studentId, type: a.type as ValidAttendanceType, note: a.note || "", date: a.date }]),
    );
